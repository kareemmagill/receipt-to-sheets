        ...
        <>
          ...
          {report.nonMembersNotPaid.lines.length > 0 && (
            <div style={{ background: "#fdecea", border: "1px solid #b00020", borderRadius: 8, padding: 12 }}>
              ...
              <DetailTable bucket={report.nonMembersNotPaid} />
            </div>
          )}

          <DetailSection title="Members Not Paid" bucket={report.membersNotPaid} />
          <DetailSection title="Members Paid" bucket={report.membersPaid} />
          <DetailSection title="Non-Members Paid" bucket={report.nonMembersPaid} />
        </>
      )}
    </main>
  );
...
function DetailSection({ title, bucket }: { title: string; bucket: DailyReportBucket }) {
  return (
    <section>
      <h2 style={{ fontSize: 16, marginBottom: 8 }}>{title}</h2>
      <DetailTable bucket={bucket} />
    </section>
  );
}

function DetailTable({ bucket }: { bucket: DailyReportBucket }) {
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={tableStyle}>
        ...
        <tbody>
          {bucket.lines.map((l) => (
            <tr key={l.customer}>
              <td style={tdStyle}>{l.slipNumbers.join(", ")}</td>
